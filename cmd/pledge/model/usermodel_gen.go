...
var (
	...
	cacheUserWalletPrefix = "cache:user:wallet:"
)
...
type (
	userModel interface {
		...
		FindOneByWallet(ctx context.Context, session sqlx.Session, wallet string) (*User, error)
	...
	User struct {
		...
		Wallet      string `db:"wallet"`       // titan的钱包地址
...
func (m *defaultUserModel) Delete(ctx context.Context, session sqlx.Session, ids ...int64) error {
	...
	for _, id := range ids {
		...
		userWalletKey := fmt.Sprintf("%s%v", cacheUserWalletPrefix, data.Wallet)
		keys = append(keys, userIdKey, userUuidKey, userWalletKey)
	}
...
func (m *defaultUserModel) FindOneByWallet(ctx context.Context, session sqlx.Session, wallet string) (*User, error) {
	userWalletKey := fmt.Sprintf("%s%v", cacheUserWalletPrefix, wallet)
	var resp User
	err := m.QueryRowIndexCtx(ctx, &resp, userWalletKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
		query := fmt.Sprintf("select %s from %s where `wallet` = ? limit 1", userRows, m.table)
		if session != nil {
			if err := session.QueryRowCtx(ctx, &resp, query, wallet); err != nil {
				return nil, err
			}
		}
		if err := conn.QueryRowCtx(ctx, &resp, query, wallet); err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
...
func (m *defaultUserModel) Insert(ctx context.Context, session sqlx.Session, datas ...*User) (sql.Result, error) {
	...
	for _, data := range datas {
		sq = sq.Values(data.Uuid, data.Wallet, data.CreatedTime, data.DeletedTime, data.Status)
...
func (m *defaultUserModel) Update(ctx context.Context, session sqlx.Session, newData *User) error {
	...
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		if session != nil {
			return session.ExecCtx(ctx, query, newData.Uuid, newData.Wallet, newData.CreatedTime, newData.DeletedTime, newData.Status, newData.Id)
		}
		return conn.ExecCtx(ctx, query, newData.Uuid, newData.Wallet, newData.CreatedTime, newData.DeletedTime, newData.Status, newData.Id)
	}, userIdKey, userUuidKey, userWalletKey)
	return err
}
