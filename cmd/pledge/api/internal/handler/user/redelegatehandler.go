...
func ReDelegateHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.ReDelegateReq
		if err := httpx.Parse(r, &req); err != nil {
			response.ParamError(w, err)
			return
		}

		l := user.NewReDelegateLogic(r.Context(), svcCtx)
		err := l.ReDelegate(&req)
		if err != nil {
			switch err.(type) {
			case merror.GzErr:
