...
func CancelUnDelegateHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.CancelUnDelegateReq
		if err := httpx.Parse(r, &req); err != nil {
			response.ParamError(w, err)
			return
		}

		l := user.NewCancelUnDelegateLogic(r.Context(), svcCtx)
		err := l.CancelUnDelegate(&req)
		if err != nil {
			switch err.(type) {
			case merror.GzErr:
